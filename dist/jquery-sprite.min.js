...
f.dataset.slide=a.config.slide;for(var g in a.config.cycles)f.appendChild(document.createElement("div"));document.body.appendChild(f)},createStyles:function(){var f=0,g="",r=document.createElement("style"),p='[data-slide="'+a.config.slide+'"]',b;b=""+(p+".slide-tmp{")+"    width: 0px;";b+="    height: 0px;";b+="    position: fixed;";b+="    left: 0px;";b+="    top: 0px;";b+="}";for(var e in a.config.cycles)b+=p+".slide-tmp div:nth-of-type("+(f+1)+"){",b+='    background: url("'+a.config.cycles[e].url+
'") 0 0 no-repeat;',b+="}",b+=p+'[data-cycle="'+f+'"]{',b+='    background: url("'+a.config.cycles[e].url+'") 0 0 no-repeat;',b+="}",g=a.config.cycles[e].url,f++;b+=p+"{";b+='    background: url("'+g+'") 0 0 no-repeat;';b+="    background-position: 0 0;";b+="}";r.innerHTML=b;document.body.appendChild(r)},createObject:function(){a.sprite=new Motio(a.elmt,a.config);a.sprite.on("frame",function(){0===this.frame&&(a.elmt.dataset.cycle=parseInt(a.elmt.dataset.cycle)+1,"3"===a.elmt.dataset.cycle&&(this.to(1,
!0),a.isEnd=!0));if(a.config.hasOwnProperty("onFrame"))a.config.onFrame(this)});if(a.config.hasOwnProperty("onPlay"))a.config.onPlay(this);if(a.config.hasOwnProperty("onPause"))a.config.onPause(this)},reset:function(){a.isEnd&&(a.elmt.dataset.cycle="0",a.isEnd=!1)},toggle:function(){a.reset();a.sprite.toggle()},play:function(){a.reset();a.sprite.play()},pause:function(){a.reset();a.sprite.pause()}};this.hasOwnProperty("play")||(a.main(t),this.__proto__.play=a.play.bind(a),this.__proto__.pause=a.pause.bind(a),
this.__proto__.toggle=a.toggle.bind(a));return this}})(jQuery);
